...
if nargin < 5
    ...
    if mfsz > 0
        uvo(:,:,1) = medfilt2(uv(:,:,1), [mfsz mfsz], 'symmetric');
        uvo(:,:,2) = medfilt2(uv(:,:,2), [mfsz mfsz], 'symmetric');    
    else
        uvo = uv;%KTH
    end
    
%     pre_filtered_uvo = uvo;
...
pad_u  = padarray(uv(:,:,1), bfhsz*[1 1], 'replicate', 'both');        
pad_v  = padarray(uv(:,:,2), bfhsz*[1 1], 'replicate', 'both');        
pad_im = padarray(im, bfhsz*[1 1], 'replicate', 'both');        
pad_occ= padarray(occ, bfhsz*[1 1], 'replicate', 'both');      
...
[M N ~] = size(im);
boundary = ceil(bfhsz/2);
for i = 1 : M
    for j = 1 : N
        if i < boundary || j < boundary || i > M - boundary || j > N - boundary
            uvo(i, j, :) = pre_filtered_uvo(i, j, :);
        end
    end
end
